"""
External Quiz Service - 外部测验生成API服务

调用外部 API 生成测验题目，替代 LLM 生成逻辑。
"""
...
class ExternalQuizService:
    ...
    def __init__(
        ...
        """
        self.api_url = api_url or settings.EXTERNAL_QUIZ_API_URL
    ...
    async def create_quiz(
        ...
        Args:
            ...
            question_count: 生成题目数量（可选，不传则由 API 自动决定）
            output_language: 输出语言（可选，如 "中文"、"英语"）
                    ...
                    "questions": [
                        {
                            "question": "问题",
                            "answer_options": [
                                {"text": "选项", "rationale": "解释", "is_correct": bool}
                            ],
                            "hint": "提示"
                        }
                    ]
        ...
        if question_count is not None:
            request_body["questionCount"] = question_count
        ...
        logger.info(f"{'='*60}")
        logger.info(f"🌐 EXTERNAL QUIZ API CALL")
        logger.info(f"{'='*60}")
        logger.info(f"📤 INPUT:")
        ...
        logger.info(f"   • QuestionCount: {question_count if question_count else 'auto'}")
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    logger.info(f"   • Questions: {len(questions)} 道")
                    for i, q in enumerate(questions, 1):
                        logger.info(f"   • Q{i}: {q.get('question', '')[:40]}...")
                    logger.info(f"{'='*60}")
                    logger.info(f"✅ EXTERNAL QUIZ API SUCCESS")
                    logger.info(f"{'='*60}")
            ...
            logger.error(f"❌ Error calling external quiz API: {e}")
            raise


# 全局单例
_service_instance: Optional[ExternalQuizService] = None


def get_external_quiz_service() -> ExternalQuizService:
    """获取全局 ExternalQuizService 实例（单例模式）"""
    global _service_instance
    if _service_instance is None:
        _service_instance = ExternalQuizService()
