...
**问题发现**: 当前实现只解决了 Plan Skill **内部**的 token 传递，不支持**跨会话**的长期记忆。

#### 🔴 当前架构盲区

```python
# app/models/memory.py (当前实现)
class ArtifactRecord(BaseModel):
    content: Dict[str, Any]  # ❌ 存储完整内容（2000+ tokens）

class SessionContext(BaseModel):
    artifact_history: List[ArtifactRecord]  # ❌ 累积所有历史
```

**问题**：
- 10 次对话 = 20000+ tokens 存在 `memory_storage/session_xxx.json`
- JSON 文件巨大（10MB+），加载慢，内存占用高
- **不支持 "infinite memory"**

#### ✅ 解决方案：引用 + S3 + 按需加载

**核心理念**: 完整内容永久保存（S3/本地），`memory_storage/*.json` 只保留轻量引用
...
class ArtifactRecord(BaseModel):
    ...
    # 🆕 外部存储引用（S3 URI 或本地路径）
    content_reference: Optional[str] = None  # "s3://..." or "user_xxx/artifact_001.json"
...
│ │   └── artifact_001.json           │
│ └── memory_storage/                 │
│     └── user_alice_session.json     │
└─────────────────────────────────────┘

开发环境（USE_S3_STORAGE=false）:
┌─────────────────────────────────────┐
│ 本地 artifacts/                      │
│ ├── user_alice/                     │
│ │   └── artifact_001.json           │
│ └── memory_storage/                 │
│     └── user_alice_session.json (引用)│
└─────────────────────────────────────┘

隔离区（无效数据）:
┌─────────────────────────────────────┐
│ quarantine/                         │
│ ├── artifact_xxx_validation_failed.json│
│ └── artifact_yyy_serialization_failed.json│
└─────────────────────────────────────┘
```

#### 🛠️ 实施计划

**Step 1**: 创建 S3 存储层 (`app/core/s3_storage.py`)
- S3StorageManager 类
- save_artifact(): 上传到 S3
- load_artifact(): 按需下载
- _validate_content(): 数据验证（< 10MB，JSON 格式）

**Step 2**: 增强 ArtifactStorage（支持 S3）
- 集成 S3StorageManager
- 优先尝试 S3，降级到本地
- 支持 S3 URI 解析

**Step 3**: 修改 ArtifactRecord 模型
- 添加 `content_reference` 字段
- 添加 `storage_type` 属性
- 向后兼容：`content` 字段保留

**Step 4**: 增强 MemoryManager
...
**Step 5**: 修改 SkillOrchestrator
- 在 `execute()` 和 `execute_stream()` 中保存 artifact
- 自动调用 `memory_manager.save_artifact()`
...
**验证机制**:
- 存储前：JSON 格式验证、大小限制（< 10MB）、必要字段检查
- 加载后：JSON 解析验证、格式验证
- 失败处理：记录详细日志、存到 `quarantine/`、发送告警（可选）

#### 📊 预期效果

| 指标 | 当前 | V2 (S3 + 验证) | 改进 |
|------|------|---------------|------|
| **memory_storage/ 大小** | 2 MB (100 次对话) | 5 KB | 99.75% ↓ |
| **数据丢失风险** | 中（本地磁盘故障） | 低（S3 高可用） | ✅ 提升 |
| **无效数据影响** | 崩溃 | 隔离（不影响正常） | ✅ 提升 |
| **加载速度** | 慢（全量加载） | 快（按需加载） | ✅ 提升 |
| **存储成本** | 免费（本地） | $0.023/GB/月 | 可接受 |
...
| 场景 | 行为 |
|------|------|
| **旧数据（content 有值）** | 继续使用 inline 存储 |
| **新数据（小内容 < 500 bytes）** | Inline 存储（快速） |
| **新数据（大内容 >= 500 bytes）** | 自动卸载到 S3/本地 |
| **S3 失败** | 降级到本地文件系统 |
| **本地失败** | 降级到 inline（内存） |
| **所有失败** | Quarantine（隔离区） |
...
### Phase 4: 验证与文档
- [x] **Phase 2 验证** (2025-11-21) ✅ 全部通过
  - ✅ 后端启动正常（无错误）
  - ✅ 配置验证（enable_artifact_offloading: false）
  - ✅ 单元测试（18/18 tests pass）
  - ✅ API 功能测试（Single Skill 正常）
  - ✅ 向后兼容性（5/5 组件 pass）
- [ ] Token 节省验证（对比测试）
- [ ] 质量保证测试（内容一致性）
- [ ] 更新文档（说明如何启用）
